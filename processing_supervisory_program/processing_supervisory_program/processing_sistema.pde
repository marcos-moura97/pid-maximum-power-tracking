...
String[] pidSeries = { "Input", "Output", "Setpoint" };
String[] angleSeries = { "Ângulo", "Ângulo", "Ângulo" };

// Data for compare
float[][] pidValues = new float[3][maxSamples];
float[][] angleValues = new float[3][maxSamples];
...
void draw() 
{
  ...
  drawChart("ângulo do servo [graus]", angleSeries, angleValues, 10, 10, 200, false, false, 0, 0, 10);
  drawChart("Parâmetros PID", pidSeries, pidValues, 10, 280, 200, true, true, 0, 360, 30);
  drawCompass(480, 5, angleValues, imgCompassPlateWhite);
//  drawCompass(480, 275, angleValues, imgCompassPlateBlack);
...
void serialEvent (Serial myPort)
{
  ...
  if (inString != null)
  {
    ...
    // PID
    pidValues[0][actualSample] = (float(list[0]));
    pidValues[1][actualSample] = (float(list[1]));
    pidValues[2][actualSample] = (float(list[2]));

    // Anfles
    angleValues[0][actualSample] = (float(list[3]));
    angleValues[1][actualSample] = (float(list[4]));
    angleValues[2][actualSample] = (float(list[5]));
    ...
    {
      nextSample(pidValues);
      nextSample(angleValues);
   } else
    {
      actualSample++;
    }
